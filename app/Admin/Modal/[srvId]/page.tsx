  ...
  return (
    <div className='flex flex-col justify-between w-full h-full px-2 md:px-20 '>
      <div className='flex flex-row items-center justify-between '>
        <h1 className='text-lg md:text-2xl font-bold text-green-600 mt-5 mb-5'>{descricao}</h1>
      ...
      <div className='flex flex-col w-full h-72 py-6 px-2 bg-gray-100 rounded-2xl overflow-hidden shadow-2xl'>
        <span className='text-lg md:text-2xl font-bold mt-6'>Objetivo:</span>
        <span className='text-lg md:text-2xl font-normal mt-6'>{objetivo}</span>
      </div>
      <div className='flex flex-row items-center w-full h-72 px-2 bg-slate-200 rounded-2xl mt-10 overflow-hidden shadow-2xl'>
        <span className='text-lg md:text-2xl font-bold mr-5'>Link de Acesso:</span>
        <Link href={link} passHref className='cursor-pointer'>
          <span className='text-lg md:text-2xl font-normal'>{link === '' ? "Serviço modalidade presencial" : link}</span>
        </Link>  
      </div>
      <div className='flex flex-col w-full h-72 py-6 px-2 bg-white rounded-2xl mt-10 mb-10 overflow-hidden shadow-2xl'>
        <span className='text-lg md:text-2xl font-bold mt-6'>Informações:</span>
        <span className='text-lg md:text-2xl font-normal mt-6'>{informacoes}</span>
      </div>
    </div>       
  )
}
